...
def get_image():
    ...
    file_name = r'C:\Users\Public\Pictures\WallPaper\%s.bmp'%local
    f = open(file_name, 'wb')
    f.write(image)
    f.close()
    return file_name
...
def main():
    if os.path.exists(r'C:\Users\Public\Pictures\WallPaper'):
        WP=get_image()
        Set_WP(WP)
    else:
        os.mkdir(r'C:\Users\Public\Pictures\WallPaper')
        WP=get_image()
        Set_WP(WP)
